...
TEST_UNSAFE(GaussianProcessPriorLinear, Factor) {
  ...
  expectH1 = numericalDerivative11(boost::function<gtsam::Vector(const Vector3&)>(
  ...
  expectH3 = numericalDerivative11(boost::function<gtsam::Vector(const Vector3&)>(
      boost::bind(&GPPrior::evaluateError, factor,
...
TEST(GaussianProcessPriorLinear, Optimization) {
  ...
  graph.add(PriorFactor<gtsam::Vector>(Symbol('x', 1), p1, model_prior));
  graph.add(PriorFactor<gtsam::Vector>(Symbol('x', 2), p2, model_prior));
  ...
  EXPECT(assert_equal(p1, values.at<gtsam::Vector>(Symbol('x', 1)), 1e-6));
  EXPECT(assert_equal(p2, values.at<gtsam::Vector>(Symbol('x', 2)), 1e-6));
  EXPECT(assert_equal(v1, values.at<gtsam::Vector>(Symbol('v', 1)), 1e-6));
  EXPECT(assert_equal(v2, values.at<gtsam::Vector>(Symbol('v', 2)), 1e-6));
}
