...
TEST(ObstacleSDFFactorGPArm, error) {
    ...
    Vector2 a(0.5, 0.5), alpha(0, 0), d(0, 0);
    Arm abs_arm(2, a, alpha, d, arm_base);

    // body info, three spheres
    BodySphereVector body_spheres;
    const double r = 0.01;
    body_spheres.push_back(BodySphere(0, r, Point3(-0.1, 0.0, 0)));
    body_spheres.push_back(BodySphere(0, r, Point3(-0.2, 0.0, 0)));
    body_spheres.push_back(BodySphere(1, r, Point3(-0.1, 0.0, 0)));
    body_spheres.push_back(BodySphere(1, r, Point3(-0.2, 0.0, 0)));
    ArmModel arm(abs_arm, body_spheres);

//    cout << "sphere_center_wrt_link " << endl << arm.sphereCenters() << endl;
    ...
    // note: Hi_act are the calculated Jacobians
    ...
    for (int i=0; i<5; i++)
    {
        Matrix H = Matrix::Zero(4, 8);
        H.block(0, 0, H1_act.rows(), H1_act.cols()) = H1_act;
        H.block(0, H1_act.cols()-1, H2_act.rows(), H2_act.cols()) = H2_act;
        H.block(0, 2*H1_act.cols()-1, H3_act.rows(), H3_act.cols()) = H3_act;
        H.block(0, 3*H1_act.cols()-1, H4_act.rows(), H4_act.cols()) = H4_act;

        Vector theta_k = concatVectors({q1, qdot1, q2, qdot2});
        Vector vec_b = H * theta_k - factor.evaluateError(q1, qdot1, q2, qdot2, H1_act, H2_act, H3_act, H4_act);
        cout << "H1 " << endl << H1_act << endl;
        cout<<"H row "<<H.rows()<<", H col "<<H.cols()<<", b size "<<vec_b.size()<<endl;
        cout << H << endl;
